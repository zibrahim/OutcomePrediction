...
def run_xgb(time_series, non_smoted_time_series,
                                   outcome, grouping, experiment_number, smote):
    ...
    xgbm=xgb.XGBClassifier(scale_pos_weight=class_weights,
                               learning_rate=0.007,
                               n_estimators=100,
                               gamma=0,
                               max_depth=4,
    ...
    plt.savefig(prediction_path+"ROC"+outcome+experiment_number+".pdf", bbox_inches='tight')

    stats_df.to_csv(stats_path + outcome+experiment_number  + "XGB.csv", index=False)

    return xgbm, false_X, false_Y, false_IDs